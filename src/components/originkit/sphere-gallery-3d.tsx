...
/* Panel follows rule 11 — 15 leaf dials, whole numbers (rule 3). Changed keys,
 * clean break, no fallbacks:
 *   sphere.baseRadius x sphere.globalScale -> radius   (one dial)
 *   sphere.itemSize                        -> size
 *   sphere.depthRandomness                 -> scatter
 *   physics.autoRotateSpeed                -> speed    (0-400 where 100 was the
 *                                            shipped rate -> 0-100 where 50 is;
 *                                            a SCALE change, so a live
 *                                            instance's spin halves)
 *   physics.magneticForce + hoverDistance  -> hover    (one dial)
 *   physics.damping                        -> damping
 *   core.coreGlow                          -> alpha on Core Color
 *   core.lineOpacity                       -> alpha on Line Color
 *   camera.minZoom/maxZoom/zoomSensitivity -> cut, frozen (Size is that dial)
 * Cut and frozen: `interaction.orbitSpeed`, `orbitDamping`, `orbitLimit`.
 * Added: `rounded`, which was the hard-coded CORNER_PCT.
 *
 * Second pass — the image list moved into a modal and the branch count became
 * its own dial:
 *   items  -> images        (same Array control, now inside the modal)
 *   —      -> branches            (NEW: how many nodes hang off the core)
 *   radius -> CUT, frozen at 12   (see below)
 *
 * Third pass: the wrapper is the folder's standard `Images` object and holds
 * ONLY the list, so a group named Images cannot contain a non-image dial.
 * `Branches` came out to top level — it is a headline control (rule 11), and
 * burying the node count behind a disclosure triangle was the wrong place for
 * it whatever the group is called.
 *
 * `Branches` is authoritative for how many nodes exist; the image list is a
 * SOURCE that cycles across them — node i draws `items[i % items.length]`, so
 * a single image fills every branch, and 3 images repeat 4× across 12. That is
 * one texture per image, not per branch.
 *
 * `Radius` is cut because it was a redundant twin of `Camera > Zoom` (rule
 * 11b): node size, scatter, core size and the hover reach are all percentages
 * OF the radius while the camera dolly is in absolute world units, so scaling
 * the radius is arithmetically the same picture as zooming in. Frozen at the
 * shipped 12 in RADIUS; nothing renders differently, and the freed leaf keeps
 * the panel at the 15-dial cap with `Branches` added.
 *
 * Fourth pass — clean break, no `?? oldKey` chains (rule 11c):
 *   damping             -> CUT, frozen at the shipped 60 in DAMPING. It was
 *                          micro-physics (rule 11b): the spring settle rate,
 *                          which no designer tunes and which reads as nothing
 *                          at all unless Hover Pull is also up.
 *   camera.perspective  -> CUT, frozen at the shipped 45° in FOV. The whole
 *                          `Camera` modal went with it — a disclosure triangle
 *                          hiding ONE dial is the layout rule 11 exists to stop.
 *   —                   -> `scale` (NEW): the framing dial the Camera group was
 *                          reaching for, at top level. Percent, 100 = the
 *                          shipped framing, and it divides the camera dolly so
 *                          the wheel range scales with it.
 *   —                   -> `direction` (NEW): which way the auto-spin turns.
 *                          An enum, not a signed Speed — the sphere has one
 *                          rotation axis and two senses, and a designer reads
 *                          "Anticlockwise" faster than "-50".
 *   speed  title Speed  -> `Spin Speed`   (key unchanged)
 *   hover  title Hover  -> `Hover Pull`   (key unchanged)
 */
...
export default function SphereGallery3D(props: SphereGallery3DProps) {
    ...
    React.useEffect(() => {
        ...
        const onWheel = (e: WheelEvent) => {
            // never hijack the wheel while the designer is editing — on the
            // Framer canvas the wheel is the editor's pan/zoom
 ...
 * Whole numbers everywhere (rule 3). Every group is a ControlType.Object and
 * every field inside it also carries a code-side fallback, because a group the
 * designer never opened arrives `undefined` (rule 8).
 */
